...
class FingerprintManager:
    ...
    def get_statistics(self):
        ...
        # Add this to your Flask application (after the existing routes)

@app.route('/api/verify_fingerprint', methods=['POST'])
def verify_fingerprint_api():
    """API endpoint to verify captured fingerprint against database"""
    try:
        # Get JSON data from request
        data = request.get_json()
        
        if not data or 'bitmap_data' not in data:
            return jsonify({
                'success': False,
        ...
        # Load the captured fingerprint image
        captured_img = manager.load_fingerprint_from_base64(bitmap_data)
        
        if captured_img is None:
            return jsonify({
                'success': False,
                'message': 'Failed to process captured fingerprint image'
            }), 400
        ...
        if not enrolled_prints:
            return jsonify({
                'success': False,
                'message': 'No enrolled fingerprints found in database'
            }), 404
        ...
        for user_id, finger_pos, stored_data in enrolled_prints:
            stored_img = manager.load_fingerprint_from_base64(stored_data)
            if stored_img is None:
                continue
            
            # Perform fingerprint matching
            is_match, similarity_score, scores = manager.match_fingerprints(captured_img, stored_img)
        ...
        status = "SUCCESS" if best_match else "FAILED"
        manager.log_verification(best_match or "Unknown", status, best_score, best_finger)
        
        if best_match:
            return jsonify({
                'success': True,
                'user_id': best_match,
                'finger_position': best_finger,
                'score': best_score,
                'match_details': match_details,
                'message': f'Verification successful for {best_match}'
            })
        else:
            return jsonify({
                'success': False,
                'message': f'No matching fingerprint found (Best score: {best_score:.4f})',
                'best_score': best_score,
                'threshold': 0.55  # You can make this configurable
            })
            
    except Exception as e:
        # Log the error (in production, use proper logging)
        print(f"Verification API error: {str(e)}")
...
def test_capture_api():
    """API endpoint to test fingerprint capture without verification"""
    try:
        data = request.get_json()
        quality = data.get('quality', 60)
        timeout = data.get('timeout', 10)
        
        # This would only work if MFScan was accessible from backend
        # Since it's not, we'll return an appropriate message
        return jsonify({
            'success': False,
            'message': 'Direct capture from backend not supported. Use frontend MFScan integration.',
            'suggestion': 'Use the frontend JavaScript MFScan integration for device communication.'
        })
...
def device_status_api():
    """API endpoint to check if MFScan service is theoretically reachable"""
    try:
        # Since we can't actually connect from backend, return status info
        return jsonify({
            'mfscan_service': 'https://localhost:8034/mfscan/',
            'backend_accessible': False,
            'frontend_accessible': True,
            'recommendation': 'Use frontend JavaScript for all device operations',
            'note': 'MFScan service only accessible from browser context'
        })
    except Exception as e:
        return jsonify({
            'success': False,
